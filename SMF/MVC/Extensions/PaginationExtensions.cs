...
namespace MVC.Extensions
{
    public static class PaginationExtensions
    {
        ...
        public static async Task<PaginatedList<TResult>> ToPagedList<T, TResult>(this IQueryable<T> source,Func<T, TResult> selectFunc,int page = 1,int pageSize = AppConfig.PageSize) where TResult : class where T : class
        {
            var count = await source.CountAsync();
            var items = await source.Skip((page - 1) * pageSize).Take(pageSize).ToListAsync();

...
public static class MegaData_NFSESpecExtensions
{
    ...
    public static ISpecificationBuilder<MegaData_NFSE> ApplyOrdering(this ISpecificationBuilder<MegaData_NFSE> builder, BaseFilter? filter = null)
    {
        // If there is no filter apply default ordering;
        if (filter is null) return builder.OrderBy(x => x.DataGeracao);
        ...
        {
            nameof(MegaData_NFSE.DataGeracao) => isAscending ? builder.OrderBy(x => x.DataGeracao) : builder.OrderByDescending(x => x.DataGeracao)
            /*,
            nameof(Pais.IdPais) => isAscending ? builder.OrderBy(x => x.IdPais) : builder.OrderByDescending(x => x.IdPais)*/,

            _ => builder.OrderByDescending(x => x.DataGeracao)
    ...
    public static ISpecificationBuilder<MegaData_NFSE> TagWith(this ISpecificationBuilder<MegaData_NFSE> builder, string tag)
    {
